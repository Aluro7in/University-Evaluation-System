...
import { calculateGpaWithJava, getJavaHealth, validateGradeWithJava } from "./javaBackend";
  ...
  grades: router({
    ...
    set: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        let gpaScale = percentageToGPA(input.grade).toFixed(2);

        // Java owns the authoritative validation/grade-scale path when it is available.
        // The TypeScript value remains as a local fallback so the web app still runs
        // when the Java container is intentionally stopped during frontend-only work.
        try {
          const javaResult = await validateGradeWithJava(input.grade);
          if (!javaResult.valid) {
            throw new TRPCError({ code: "BAD_REQUEST", message: "Java backend rejected the grade" });
          }
          gpaScale = javaResult.gpa.toFixed(2);
        } catch (error) {
          console.warn("[JavaBackend] Grade validation unavailable; using TypeScript fallback", error);
        }
        ...
        if (enrollment.courseId !== input.courseId) {
          throw new TRPCError({ code: "BAD_REQUEST", message: "Course does not match enrollment" });
        }
    ...
    calculateGPA: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        let gpa = calculateStudentGPA(student.type as "engineering" | "management", courseGrades);
        let calculationEngine = "typescript-fallback";

        try {
          const javaResult = await calculateGpaWithJava(student.type as "engineering" | "management", courseGrades);
          gpa = javaResult.gpa;
          calculationEngine = "java-spring-boot";
        } catch (error) {
          console.warn("[JavaBackend] GPA service unavailable; using TypeScript fallback", error);
        }
    ...
    transcript: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        try {
          ...
          console.warn("[JavaBackend] Transcript GPA unavailable; using TypeScript fallback", error);
        }
