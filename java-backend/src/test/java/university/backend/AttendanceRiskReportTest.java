...
import university.backend.report.AttendanceRiskReport;
import static org.junit.jupiter.api.Assertions.*;

class AttendanceRiskReportTest {
    @Test void createsUsefulMetrics(){
        AttendanceRiskReport report=new AttendanceRiskReport("demo",90,80,70);
    ...
    @Test void exposesStableSummary(){
        AttendanceRiskReport report=new AttendanceRiskReport("demo",95,90,85);
    ...
    @Test void combinesMetrics(){
        AttendanceRiskReport first=new AttendanceRiskReport("demo",50,60,70);
        AttendanceRiskReport second=new AttendanceRiskReport("demo",10,20,30);
        AttendanceRiskReport combined=first.add(second);
        assertEquals(120,combined.total(),0.001);
        assertTrue(combined.equalsMetrics(new AttendanceRiskReport("demo",60,80,100),0.001));
        assertEquals(120,combined.scale(1).total(),0.001);
    }

    @Test void handlesEmptyAndTargets(){
        AttendanceRiskReport empty=AttendanceRiskReport.empty("empty");
