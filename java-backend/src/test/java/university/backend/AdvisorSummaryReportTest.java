...
import university.backend.report.AdvisorSummaryReport;
import static org.junit.jupiter.api.Assertions.*;

class AdvisorSummaryReportTest {
    @Test void createsUsefulMetrics(){
        AdvisorSummaryReport report=new AdvisorSummaryReport("demo",90,80,70);
    ...
    @Test void exposesStableSummary(){
        AdvisorSummaryReport report=new AdvisorSummaryReport("demo",95,90,85);
    ...
    @Test void combinesMetrics(){
        AdvisorSummaryReport first=new AdvisorSummaryReport("demo",50,60,70);
        AdvisorSummaryReport second=new AdvisorSummaryReport("demo",10,20,30);
        AdvisorSummaryReport combined=first.add(second);
        assertEquals(120,combined.total(),0.001);
        assertTrue(combined.equalsMetrics(new AdvisorSummaryReport("demo",60,80,100),0.001));
        assertEquals(120,combined.scale(1).total(),0.001);
    }

    @Test void handlesEmptyAndTargets(){
        AdvisorSummaryReport empty=AdvisorSummaryReport.empty("empty");
