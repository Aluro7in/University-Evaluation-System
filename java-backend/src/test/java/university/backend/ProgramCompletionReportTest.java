...
import university.backend.report.ProgramCompletionReport;
import static org.junit.jupiter.api.Assertions.*;

class ProgramCompletionReportTest {
    @Test void createsUsefulMetrics(){
        ProgramCompletionReport report=new ProgramCompletionReport("demo",90,80,70);
    ...
    @Test void exposesStableSummary(){
        ProgramCompletionReport report=new ProgramCompletionReport("demo",95,90,85);
    ...
    @Test void combinesMetrics(){
        ProgramCompletionReport first=new ProgramCompletionReport("demo",50,60,70);
        ProgramCompletionReport second=new ProgramCompletionReport("demo",10,20,30);
        ProgramCompletionReport combined=first.add(second);
        assertEquals(120,combined.total(),0.001);
        assertTrue(combined.equalsMetrics(new ProgramCompletionReport("demo",60,80,100),0.001));
        assertEquals(120,combined.scale(1).total(),0.001);
    }

    @Test void handlesEmptyAndTargets(){
        ProgramCompletionReport empty=ProgramCompletionReport.empty("empty");
